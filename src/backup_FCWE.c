...
struct char_Fcorner{
    int *Fcorner, Fcorner_size;  // Fcorner[i]  : the i -th Fcorner   Fcorner_size, the number of Fcorners
};
...
char train_file[MAX_STRING], comp_file[MAX_STRING],char2comp_file[MAX_STRING], Fcorner_file[MAX_STRING], char2Fcorner_file[MAX_STRING];
char output_word[MAX_STRING], output_char[MAX_STRING], output_comp[MAX_STRING], output_Fcorner[MAX_STRING];
struct vocab_word *vocab;
struct char_component char2comp[CHAR_SIZE];
struct char_Fcorner char2Fcorner[CHAR_SIZE];
struct components *comp_array;
struct Fcorner *Fcorner_array;
...
int *vocab_hash;
long long vocab_max_size = 1000, vocab_size = 0, comp_max_size = COMP_SIZE, Fcorner_max_size = FCORNER_SIZE, Fcorner_size = 0, comp_size = 0, layer1_size = 200;
long long train_words = 0, word_count_actual = 0, file_size = 0;
real alpha = 0.025, starting_alpha, sample = 0;
real *syn0, *syn1neg, *expTable, *synchar, *syncomp, *synFcorner;
clock_t start;
...
void DestroyVocab(){
    ...
    for (a = 0; a < CHAR_SIZE; a++){
        if (char2Fcorner[a].Fcorner != NULL)
            free(char2Fcorner[a].Fcorner);
    }
	...
	free(comp_array);
    for(a = 0; a < Fcorner_size; a++){
        if(Fcorner_array[a].Fcorner_val != NULL)
            free(Fcorner_array[a].Fcorner_val);
    }
    free(Fcorner_array);
}
...
void ReadFcorner() {
    FILE *fin;
    fin = fopen(Fcorner_file, "rb");
    if (fin == NULL){
        fprintf(stderr,"ERROR : Fcorner file not found!\n");
        exit(1);
    }
    while(1){
        char word[MAX_STRING];
        ReadWord(word, fin);
        if (feof(fin)) break;
        if(Fcorner_size + 2 >= Fcorner_max_size){
            Fcorner_max_size += FCORNER_SIZE;
            Fcorner_array = (struct Fcorner *)realloc(Fcorner_array,Fcorner_max_size * sizeof(struct Fcorner));
        }
        unsigned int word_len = strlen(word) + 1;
        Fcorner_array[Fcorner_size].Fcorner_val = (char *)calloc(word_len, sizeof(char));
        strcpy(Fcorner_array[Fcorner_size].Fcorner_val, word);
        Fcorner_size++;
    }
    fclose(fin);
}
...
// find the index of a Fcorneronent in the Fcorner array
int GetFcornerIndex(char *Fcorner){
  for(int i = 0; i < Fcorner_size; i++){
    if(strcmp(Fcorner, Fcorner_array[i].Fcorner_val) == 0)
      return i;
  }
  return -1;
}
...
//Read char2Fcorner and Fcornerarray from file
void LearnCharFcornerFromFile() {
    FILE *fin;
    fin = fopen(char2Fcorner_file,"rb");
    if(fin == NULL){
        fprintf(stderr,"ERROR: char2Fcorner file not found!\n");
        exit(1);
    }
    char *line = NULL;
    size_t len = 0;
    ssize_t read;
    while ((read = getline(&line, &len, fin)) != -1){
        //parse the line , get characters and its Fcorner
        int num = (strlen(line) - 2) / 3 - 1;
        char *save_ptr;
        char *pch = strtok_r(line, " \n",&save_ptr);
        wchar_t wstr[MAX_STRING];
        unsigned int wlen = mbstowcs(wstr, pch, MAX_STRING);
        int id = wstr[0] - MIN_CHINESE;
        char2Fcorner[id].Fcorner_size = num;
        char2Fcorner[id].Fcorner = calloc(char2Fcorner[id].Fcorner_size, sizeof(int));           //
        int tmp_cnt = 0;
        pch = strtok_r(NULL," \n",&save_ptr);
        while(pch != NULL){
            int pch_index = GetFcornerIndex(pch);
            if(pch_index != -1)
                char2Fcorner[id].Fcorner[tmp_cnt++] = pch_index;
            pch = strtok_r(NULL," \n",&save_ptr);
        }
    }
    if(line)
...
void InitNet(){
  ...
  a = posix_memalign((void **)&synFcorner, 128, (long long)Fcorner_size * layer1_size * sizeof(real));
  if (synFcorner == NULL) {printf("Memory allocation failed\n"); exit(1);}
  ...
  for (b = 0; b < layer1_size; b++) for (a = 0; a < Fcorner_size; a++)
    synFcorner[a * layer1_size + b] = (rand() / (real)RAND_MAX - 0.5) / layer1_size;
}
void DestroyNet(){
  if (syn0 != NULL){
    free(syn0);
...
void *TrainModelThread(void *id) {
  long long a, b, c, d, e, char_id, comp_id, Fcorner_id, word, last_word, sentence_length = 0, sentence_position = 0;
  ...
  long long *Fcorner_id_list = calloc(MAX_SENTENCE_LENGTH, sizeof(long long));
  int char_list_cnt = 0, comp_list_cnt = 0, Fcorner_list_cnt=0;
  ...
  real *neuFcorner = (real *)calloc(layer1_size,sizeof(real));
  real *neuFcorner_grad = (real *)calloc(layer1_size,sizeof(real));
  ...
  while (1) {
    ...
    for (c = 0; c < layer1_size; c++) neuFcorner[c] = 0;
    for (c = 0; c < layer1_size; c++) neuFcorner_grad[c] = 0;
    next_random = next_random * (unsigned long long)25214903917 + 11;
    ...
    for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
      ...
      for (c = 0; c < vocab[last_word].character_size; c++) {
        ...
        if (pos_type == 1 || pos_type == 3) {
          ...
          for (d = 0; d < char2Fcorner[char_id].Fcorner_size; d++) {
              Fcorner_id = char2Fcorner[char_id].Fcorner[d];
              Fcorner_id_list[Fcorner_list_cnt++] = Fcorner_id;
              for (e = 0; e < layer1_size; e++) neuFcorner[e] += synFcorner[e + Fcorner_id * layer1_size];
          }
        }
      }
      cw++;
    ...
    if (cw) {
      if (average_sum == 1) {       // the context is represented by the average of the surrounding vectors
        for (c = 0; c < layer1_size; c++) {
          ...
          if (Fcorner_list_cnt > 0)
            neuFcorner[c] /= Fcorner_list_cnt;
        }
      }
      // negative sampling
      for (d = 0; d < negative + 1; d++) {
        ...
        if (join_type == 1) {    // sum loss composition model
          ...
          for (c = 0; c < layer1_size; c++) {
            ...
            neuFcorner_grad[c] += g4 * syn1neg[c + l2];
          }
          //update syn1neg
          for (c = 0; c < layer1_size; c++)
            syn1neg[c + l2] += g1 * neu1[c] + g2 * neuchar[c] + g3 * neucomp[c] + g4 * neuFcorner[c];
        }
        else if (join_type == 2) { // average context composition model
          real f = 0, g = 0;
          for (c = 0; c < layer1_size; c++)
            f += (neu1[c] + neuchar[c] + neucomp[c] + neuFcorner[c]) * syn1neg[c + l2] ;
          if (f > MAX_EXP)
            g = (label - 1) * alpha;
          else if (f < -MAX_EXP)
            g = (label - 0) * alpha;
          ...
          for (c = 0; c < layer1_size; c++) {
            ...
            neuFcorner_grad[c] += g * syn1neg[c + l2];
            neuchar_grad[c] += g * syn1neg[c + l2];
          }
          for (c = 0; c < layer1_size; c++)
            syn1neg[c + l2] += g * (neu1[c] + neuchar[c] + neucomp[c] + neuFcorner[c]);
        }
      }
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        for (a = 0; a < Fcorner_list_cnt; a++) {
            Fcorner_id = Fcorner_id_list[a];
            for (c = 0; c < layer1_size; c++){
                if (average_sum == 1)
                    synFcorner[c + Fcorner_id * layer1_size] += neuFcorner_grad[c] / Fcorner_list_cnt;
                else
                    synFcorner[c + Fcorner_id * layer1_size] += neuFcorner_grad[c];
            }
        }
    }
    sentence_position++;
...
void TrainModel(){
  ...
  if (strlen(output_Fcorner)){
    fo = fopen(output_Fcorner, "wb");
    if (fo == NULL){
        fprintf(stderr, "Cannot open %s: permission denied\n", output_Fcorner);
    }
    fprintf(fo, "%lld %lld\n", Fcorner_size, layer1_size);
    for(a = 0; a < Fcorner_size; a++){
        fprintf(fo, "%s ", Fcorner_array[a]);
        if (binary)
            for (b = 0; b < layer1_size; b++) fwrite(&synFcorner[a * layer1_size + b], sizeof(real), 1, fo);
        else
            for (b = 0; b < layer1_size; b++) fprintf(fo, "%lf ", synFcorner[a * layer1_size + b]);
        fprintf(fo, "\n");
    }
    fclose(fo);
}
...
int main(int argc, char **argv) {
  ...
  if(argc == 1){
    printf("WORD VECTOR estimation toolkit v 0.1b\n\n");
      ...
      printf("\t-output-Fcorner <file>\n");
      printf("\t\tUse <file> to save the resulting Fcorner vectors\n");
      printf("\t-size <int>\n");
      ...
      printf("\t\tObtain the mapping between characters and their Fcorner from <file>\n");
      printf("\t-Fcorner <file>\n");
      printf("\t\tUse 4Corner list from <file>\n");
      printf("\t-char2Fcorner <file>\n");
      printf("\t\tObtain the mapping between characters and their 4Corner from <file>\n");
      ...
      printf("\nExamples:\n");
      printf("./jwe -train wiki_process.txt -output-word word_vec -output-char char_vec -output-comp comp_vec -output-Fcorner Fcorner_vec -size 200 -window 5 -sample 1e-4 -negative 10 -iter 100 -threads 8 -min-count 5 -alpha 0.025 -binary 0 -comp ../subcharacter/comp.txt -char2comp ../subcharacter/char2comp.txt -Fcorner ../subcharacter/Fcorner.txt -char2Fcorner ../subcharacter/char2Fcorner.txt -join-type 1 -pos-type 1 -average-sum 1\n\n");
      return 0;
  }
  output_word[0] = 0;
  ...
  if ((i = ArgPos((char *)"-output-Fcorner", argc, argv)) > 0) strcpy(output_Fcorner, argv[i + 1]);
  if ((i = ArgPos((char *)"-Fcorner", argc, argv)) > 0) strcpy(Fcorner_file, argv[i + 1]);
  if ((i = ArgPos((char *)"-char2Fcorner", argc, argv)) > 0) strcpy(char2Fcorner_file, argv[i + 1]);
  ...
  Fcorner_array = (struct Fcorner *)calloc(Fcorner_max_size, sizeof(struct Fcorner));
